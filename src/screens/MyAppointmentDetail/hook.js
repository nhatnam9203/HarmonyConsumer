...
export default function hook(props) {
  ...
  let total =
    ...
    totalPrice(services, extras, products);
  ...
  const callBackEdit = () => {
    ...
    dispatch(
      actions.bookingAction.selectTime(moment(fromTime).format('HH:mm')),
    );
  ...
  const fetchDataStoreDetail = () => {
    dispatch(
      actions.appointmentAction.getCategoryByStore(merchantId, token, () => {}),
    );
    dispatch(
      actions.appointmentAction.getServiceByStore(merchantId, token, () => {}),
    );
