...
export default function Appointment(props) {
  ...
  const viewAppointmentDetail = () => {
    ...
    dispatch(
      actions.appointmentAction.getDetailAppointment(token, appointmentId),
    );
    ...
    <TouchableOpacity
      ...
      <View>
        ...
        <Info
          ...
          address={address}
          renderImgStore={renderImgStore}
        />
        <View style={styles.bottom}>
          <Text style={styles.txtBottom}>
            {textBottom(services, products, extras)}
          </Text>
    ...
    case 'unconfirm':
      return (
        <View
          ...
          <Text style={[styles.txtStatus, { color: '#444' }]}>
    ...
    case 'checkin':
      return (
        <View style={[styles.wrapStatus, { backgroundColor: '#0764B0' }]}>
          ...
          <Text style={[styles.txtStatus, { color: 'white' }]}>
            {convertStatus(status)}
          </Text>
        </View>
      );
...
const styles = StyleSheet.create({
  container: status => {
    return {
      width: scaleWidth(94),
      borderRadius: 6,
      borderWidth: status === 'unconfirm' ? 1.5 : 1,
      borderColor:
        status === 'unconfirm'
          ? '#ffff60'
