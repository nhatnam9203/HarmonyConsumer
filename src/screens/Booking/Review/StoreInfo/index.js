...
export default function StoreInfo(props) {
  const { goToSelectDate, merchant, fromTime, services, extras, status, isEditAppointment } = props;
  ...
  const date =
    merchant_detail.timezone && merchant_detail.timezone !== "0"
      ? moment(fromTime).tz(merchant_detail.timezone.substring(12)).format("dddd, MMMM DD YYYY")
      : moment(fromTime).format("dddd, MMMM DD YYYY");

  const renderImgStore = banners.length > 0 ? { uri: banners[0].imageUrl } : images.bannerMerchant;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.rowInfo}>
        ...
        <View style={styles.infoRight}>
          <BusinessName name={businessName} isEditAppointment={isEditAppointment} />
