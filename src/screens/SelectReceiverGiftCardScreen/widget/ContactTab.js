...
import { StyleSheet, Image, View, FlatList, Platform, PermissionsAndroid } from "react-native";
import Contacts from "react-native-contacts";
import { useDispatch, useSelector } from "react-redux";

import actions from "@redux/actions";
import { CheckBox, Text, Form } from "components";

import { scaleSize, formatPhoneNumberContact, formatPhoneStandard, scaleWidth } from "utils";
...
export default function creditCard({ onNextScreen }) {
  ...
  const result_filter = useSelector((state) => state.buygiftReducer.result_filter);
  ...
  React.useEffect(() => {
    Platform.OS === "android" ? requestPerContactsAndroid() : requestPerContacts();
  }, []);
  ...
  const getAllContacts = async () => {
    Contacts.getAll((err, contacts) => {
      let phones = contacts
        .filter((item) => !isEmpty(item.phoneNumbers) && item.phoneNumbers.length > 0)
        .map((item) => {
          if (item.phoneNumbers[0]) {
            const number = replaceAll(item.phoneNumbers[0].number, " ", "");
            let phone;
            if (number.includes("+84")) {
              phone = number?.substring(1, 3) + formatPhoneStandard(number?.substring(3));
            } else if (number.includes("+1")) {
              phone = number.substring(1, 2) + formatPhoneStandard(number.substring(2));
            } else if (number.charAt(0) == "8" && number.charAt(1) == "4") {
              phone = number?.substring(0, 2) + formatPhoneStandard(number?.substring(2));
            } else if (number.charAt(0) == "1") {
              phone = number?.substring(0, 1) + formatPhoneStandard(number?.substring(1));
      ...
      if (!isEmpty(phones)) dispatch(actions.buygiftAction.get_contacts(token, phones));
    });
  };
  ...
  return (
    <View style={{ flex: 1, paddingHorizontal: scaleWidth(3) }}>
      <FlatList
        ...
        contentContainerStyle={{ paddingTop: scaleSize(20), paddingBottom: scaleSize(50) }}
