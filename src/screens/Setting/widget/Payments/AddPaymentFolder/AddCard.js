        ...
        <View>
          ...
          <View style={{ flexDirection: "row", marginTop: scaleWidth(3) }}>
            <Menu
              ...
              ref={refMenuMonth}
              button={
                <TouchableOpacity onPress={showMenuMonth} style={styles.itemSelect}>
                  <Text style={styles.txtNormal}>{month_expire}</Text>
                  <AntDesign name="down" size={scaleWidth(2.8)} color={"#404040"} />
                </TouchableOpacity>
              }>
              <ScrollView>
                {months.map((month, index) => (
                  <MenuItem key={`${month}${index}`} onPress={() => hideMenuMonth(month)}>
                    {month}
                  </MenuItem>
                ))}
              </ScrollView>
            ...
            <Menu
              style={{ height: scaleWidth(70), width: scaleWidth(25), marginLeft: scaleWidth(3) }}
              ref={refMenuYear}
              button={
                <TouchableOpacity
              ...
              <ScrollView>
                {years.map((year, index) => (
                  <MenuItem key={`${year}${index}`} onPress={() => hideMenuYear(year)}>
                    {year}
                  </MenuItem>
                ))}
              </ScrollView>
